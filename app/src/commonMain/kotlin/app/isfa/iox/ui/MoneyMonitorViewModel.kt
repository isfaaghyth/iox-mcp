...
class MoneyMonitorViewModel(private val useCase: GetGeminiUseCase) : ViewModel() {
    ...
    private fun shouldGeminiRequest(model: ImageIntentData) {
        val prompt = "You're an receipt information extractor. " +
                "" +
                "Please extract some of information from attachment image. " +
                "First, extract the total estimation exact number in Rupiah." +
                "Second, extract the merchant name, and estimate what category should be based on information you get." +
                "Finally, you only provides me a json string with this format { \"merchant_name\": \"\", \"total_rupiah\": \"\", \"category\": \"\" }"
